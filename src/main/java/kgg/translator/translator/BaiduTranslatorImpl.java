...
public class BaiduTranslatorImpl extends BaiduTranslator {
    ...
    public void register(LiteralArgumentBuilder<FabricClientCommandSource> node) {
        ...
        Command<FabricClientCommandSource> s = context -> {


            return 0;
        };
        var a = ClientCommandManager.argument("qps", IntegerArgumentType.integer(0)).executes(s);
        var b = ClientCommandManager.argument("appId", StringArgumentType.word()).executes(s);
        var c = ClientCommandManager.argument("appKey", StringArgumentType.word()).executes(s);

        var d = ClientCommandManager.literal("qps");
        var e = ClientCommandManager.literal("appId");
        var f = ClientCommandManager.literal("appKey");
        ...
        node.then(d).then(e).then(f);

//        var a = ClientCommandManager.literal("qps").then()
